...
const AngularExporter = ({ shapes, stageSize }) => {
  ...
  const generateAngularCode = () => {
    ...
    // Generar imports
    let imports = `import { Component${isPage ? '' : ', Input'} } from '@angular/core';\n`;
    if (shapes.some(shape => shape.type === 'text')) {
      imports += `import { CommonModule } from '@angular/common';\n`;
    }

    // Generar template
    const template = shapes.map((shape) => {
      const style = [];
      const classes = [];
      ...
      switch (shape.type) {
        case 'rectangle':
          ...
          return `<div class="${classes.join(' ')}" style="${style.join('; ')}"></div>`;
        ...
        case 'text':
          ...
          if (shape.textDecoration === 'underline') style.push(`text-decoration: underline`);
          if (shape.textAlign) style.push(`text-align: ${shape.textAlign}`);
          return `<p class="${classes.join(' ')}" style="${style.join('; ')}">${shape.text || ''}</p>`;
        ...
        case 'image':
          ...
          return `<img src="${shape.src || ''}" class="${classes.join(' ')}" style="${style.join('; ')}" alt="Imported image"/>`;
    ...
    const metadata = {
      ...
      standalone: true,
      imports: shapes.some(shape => shape.type === 'text') ? 'CommonModule' : undefined,
      template: `
    <div class="container"${isPage ? '' : ' [style.width.px]="width" [style.height.px]="height"'}>
      ${template}
    </div>`,
      styles: [`
    .container {
...
export class ${className} {
  ${isPage ? '' : `@Input() width: number = ${stageSize.width};
  ...
  const handleExport = () => {
    ...
    const fileName = exportMode === 'page'
        ...
        <div className="space-y-3">
          <div className="flex gap-2 mb-3">
            <button
              ...
              className={`flex-1 py-2 px-3 text-sm rounded-lg ${exportMode === 'component'
            ...
            <button
              ...
              className={`flex-1 py-2 px-3 text-sm rounded-lg ${exportMode === 'page'
                  ? 'bg-blue-500 text-white'
                  : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
            >
          ...
          {exportMode === 'component' && (
            <div className="mb-3">
              <label className="block text-xs text-gray-500 mb-1">Nombre del Componente</label>
              <input
                type="text"
                value={componentName}
                onChange={(e) => setComponentName(e.target.value.replace(/[^a-zA-Z]/g, ''))}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 p-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg"
          >
